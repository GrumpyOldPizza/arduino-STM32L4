			...
			uint8_t cfgidx)
{
  ...
  else
  {
    ...
    ((USBD_CDC_ItfTypeDef *)pdev->pUserData[0])->Init(pdev);
			 ...
			 USBD_SetupReqTypedef *req)
{
  ...
  case USB_REQ_TYPE_CLASS :
    if (req->wLength)
    {
      ...
      {
        ((USBD_CDC_ItfTypeDef *)pdev->pUserData[0])->Control(req->bRequest,
							     (uint8_t *)hcdc->data,
							     req->wLength);
...
uint8_t  USBD_CDC_DataIn (USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if(pdev->pClassData[0] != NULL)
  {
    ...
    ((USBD_CDC_ItfTypeDef *)pdev->pUserData[0])->Transmit();
...
{      
  ...
  if(pdev->pClassData[0] != NULL)
  {
    ((USBD_CDC_ItfTypeDef *)pdev->pUserData[0])->Receive(hcdc->RxBuffer, hcdc->RxLength);
...
{ 
  ...
  if((pdev->pUserData[0] != NULL) && (hcdc->CmdOpCode != 0xFF))
  {
    ((USBD_CDC_ItfTypeDef *)pdev->pUserData[0])->Control(hcdc->CmdOpCode,
							 (uint8_t *)hcdc->data,
							 hcdc->CmdLength);
      hcdc->CmdOpCode = 0xFF; 

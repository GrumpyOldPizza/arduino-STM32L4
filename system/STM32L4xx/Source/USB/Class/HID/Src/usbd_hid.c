			...
			uint8_t cfgidx)
{
  ...
  ((USBD_HID_ItfTypeDef *)pdev->pUserData[2])->Init(pdev);
			  ...
			  uint8_t cfgidx)
{
  ...
  {
    ((USBD_HID_ItfTypeDef *)pdev->pUserData[2])->DeInit();
			 ...
			 USBD_SetupReqTypedef *req)
{
  ...
  case USB_REQ_TYPE_CLASS :  
    ...
    case HID_REQ_GET_REPORT:
      len = ((USBD_HID_ItfTypeDef *)pdev->pUserData[2])->GetReport(req->wValue >> 8, req->wValue & 0xff, (uint8_t*)&hhid->ReportData[0]);
      USBD_CtlSendData (pdev, 
			  ...
			  uint8_t epnum)
{
  ...
  ((USBD_HID_ItfTypeDef *)pdev->pUserData[2])->TxDone();
			   ...
			   uint8_t epnum)
{
  ...
  ((USBD_HID_ItfTypeDef *)pdev->pUserData[2])->RxReady((uint8_t*)&hhid->EventData[0], USBD_LL_GetRxDataSize (pdev, epnum));
...
uint8_t USBD_HID_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  if (hhid->SetReportBusy)
  {
    ...
    ((USBD_HID_ItfTypeDef *)pdev->pUserData[2])->SetReport(hhid->ReportType, hhid->ReportID, (uint8_t*)&hhid->ReportData[0], USBD_LL_GetRxDataSize (pdev, 0));
  }
